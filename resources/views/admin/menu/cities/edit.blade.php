...
@section('content')
    <div class="container-fluid">
        ...
        <div class="py-3 py-lg-4">
            <div class="row">
                <div class="col-lg-6">
                    <h4 class="page-title mb-0">Edit City</h4>
                </div>
                <div class="col-lg-6">
        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <form id="cityForm" action="{{ route('admin.cities-update') }}" enctype="multipart/form-data"
                            method="POST">
                            @csrf
                                            ...
                                            <select class="form-control form-select" name="state_id" id="state_id">
                                                @foreach ($states as $main)
                                                    <option value="{{ $main->id }}"
                                                        {{ $data->state_id == $main->id ? 'selected' : '' }}>
                                                        {{ $main->name }}
                                                    </option>
                                                @endforeach
                                            </select>
...
@section('script')
    ...
    <script>
        ...
        $('#citySubmit').click(function(e) {
            ...
            var name = $('#name').val();
            if (name === '') {
                Swal.fire('Validation Error', 'Please enter city name.', 'error');
                return;
            }
            if (slug === '') {
